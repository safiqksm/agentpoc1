...
import { OktaTokenPanel } from "./TokenPanel";

const MAX_PROMPT_LENGTH = 2000;

export default function ChatPage({ oktaUser, oktaIdToken }) {
  ...
  async function handleSubmit(e) {
    ...
    setResponse(null);
    try {
      const result = await sendPrompt(prompt.trim(), oktaIdToken);
      setResponse(result);
    } catch (err) {
    ...
    <div style={{ flex: 1, padding: '24px', maxWidth: 860, width: '100%', margin: '0 auto', boxSizing: 'border-box' }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ color: 'var(--az-text-secondary)', fontSize: 13, marginTop: 4 }}>
          {oktaUser
            ? <>Signed in via <strong>Okta</strong> as <strong>{oktaUser.email ?? oktaUser.name}</strong> — XAA token exchange active</>
            : <>Sign in with Okta to enable XAA token exchange</>
          }
        </p>
      </div>

      {/* Okta ID token panel — collapsed by default, shows claims + raw JWT */}
      <OktaTokenPanel idToken={oktaIdToken} />
      ...
      <div style={card}>
        ...
        <form onSubmit={handleSubmit}>
          <textarea
            ...
            placeholder="e.g. What department does user 00u1abc work in?"
            rows={4}
            style={{
              width: '100%',
              padding: '10px 12px',
      ...
      {response && (
        <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {/* Footer metadata */}
          <div style={{ fontSize: 12, color: 'var(--az-text-secondary)', padding: '0 4px', display: 'flex', gap: 16, alignItems: 'center' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
              <span style={{ fontWeight: 600, color: 'var(--az-blue-dark)' }}>Model</span>
              <code style={{ fontSize: 12, background: 'var(--az-blue-light)', padding: '1px 6px', borderRadius: 3 }}>{response.model}</code>
            </span>
            <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
              <span style={{ fontWeight: 600, color: 'var(--az-blue-dark)' }}>Token preview</span>
              <code style={{ fontSize: 12, background: 'var(--az-blue-light)', padding: '1px 6px', borderRadius: 3 }}>{response.token_preview}</code>
            </span>
          </div>
